				...
				?>
					<div class="checkbox-inline">
						<label>
							...
							<?= $add_on_row["label"] ?>
							<input type="number" class="form-control add_on_qty" id="<?= $add_on_row['label_id'] ?>_qty" name="<?= $add_on_row['label_id'] ?>_qty_<?= $row['product_id']?>" value="1" onchange="changeSelection('<?= $row['product_id'] ?>','<?= $add_on_row['label_id']; ?>',this,false,'<?= $add_on_row['label']; ?>',<?= $add_on_row['value']; ?>)">
						</label>
					</div>
					<?php
